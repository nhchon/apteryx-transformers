...
class PickleDataset(Dataset):
    ...
    def __getitem__(self, item):
        d = self.tokenizer(self.txt[item],
                           padding='max_length',
                           truncation=True,
                           max_length=self.block_size,
                           return_tensors='pt',
...
class ExpandedDataset(Dataset):
    ...
    def __init__(self, tokenizer, dataset, class_labels, block_size: int, limit=None):
        ...
        for item in tqdm(dataset[:limit]):
            ...
            item_txt = '\n'.join([item['abs_text'], item['desc_text'], item['claims_text']])
            tokenized_txt = self.tokenizer(item_txt, return_tensors='pt')  # input_ids, attention_mask
            input_ids = tokenized_txt['input_ids']
            q_tokens = input_ids.shape[1]
            # print('tokens in pat:', q_tokens)
            chunks = [input_ids[0, i:i + block_size]
                      for i in range(0, q_tokens, block_size)]
            for chunk in chunks:
                # print(chunk.shape)
...
class PickleDatasetByClass(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # Map int item to file path
        chunk_file = os.path.join(self.data_dir, self.class_to_patent_map[_class][_class_idx])
        with open(chunk_file, 'rb') as f:
            txt, label = pickle.load(f)
            assert int(_class) == int(label), f"Label {label} doesn't match Class {_class}!"
